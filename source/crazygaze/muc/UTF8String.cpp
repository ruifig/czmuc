...
namespace cz
{
	...
	UTF8String& UTF8String::appendCodepoint(uint32_t codepoint)
	{
		char buf[5];
		char* start=&buf[0];
	...
	size_t UTF8String::find(uint32_t codepoint, size_t offset) const
	{
	...
	// UTF8String iterator
	//
	uint32_t UTF8String::Iterator::operator * () const
	{
		const char* temp = mIt;
